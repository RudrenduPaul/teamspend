...
| Tests | 60 passing, 98.2% line coverage |
...
Yes, for OpenCode -- see [OpenCode: local-only, no API key needed](#opencode-local-only-no-api-key-needed) above; it reads OpenCode's local session logs directly, no API key required. GitHub Copilot is next on the roadmap; a CSV-import fallback already lets you supply spend data for any other tool in the meantime, using the same `date,user_email,cost_usd,is_estimated` schema documented above.
